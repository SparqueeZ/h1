...
class HomePageTeacherScreen extends StatefulWidget {
  ...
  @override
  _HomePageTeacherScreenState createState() => _HomePageTeacherScreenState();
}
...
class _HomePageTeacherScreenState extends State<HomePageTeacherScreen > {
  ...
  Widget _buildLessonCard(Lesson lesson, bool isUpcoming) {
    ...
    String sessionToken = lesson.sessionToken.toString();
    String formattedSessionToken = '#${sessionToken}';
    String promotions = lesson.promotions.join(', ');
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () {
                          _showStudentsPopup(context, lesson.students);
                        },
                        child: SizedBox(
                          height: 40,
                          child: Stack(
                            children: [
                              for (int i = 0; i < lesson.students.length; i++)
                                if (i < 4)
                                  Positioned(
                                    left: (i * (1 - 0.4) * 24).toDouble(),
                                    child: CircleAvatar(
                                      radius: 16,
                                      backgroundColor: Colors.grey[700],
                                      child: Container(
                                        ...
                                        child: Center(
                                          child: Text(
                                            '${lesson.students[i].firstname[0].toUpperCase()}${lesson.students[i].lastname[0].toUpperCase()}',
                                            style: TextStyle(
                                              color: Colors.white,
                              ...
                              if (lesson.students.length > 4)
                                Positioned(
                                  left: (4 * (1 - 0.4) * 24).toDouble(),
                                  child: CircleAvatar(
                                    radius: 16,
                                    backgroundColor: Colors.grey[500],
                                    child: Text(
                                      '+${lesson.students.length - 4}',
                                      style: TextStyle(
                                        color: Colors.white,
                                        fontWeight: FontWeight.bold,
                                        fontSize: 12,
  ...
  void _showStudentsPopup(BuildContext context, List<Student> students) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          backgroundColor: const Color(0xFF141B25),
          title: Text('Liste des étudiants', style: TextStyle(color: Colors.white)),
          content: Container(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  trailing: Icon(
                    ...
                    color: student.isPresent ? successColor : errorColor,
                  ),
                );
              },
            ),
